...
#-------------------------------------------------------------------------------
IMAGES_TO_DONWLOAD = 20000 #Number of images to download. MUST BE A MULTIPLE OF 50.
...
def downloadActors():
	...
	for i in range(1, IMAGES_TO_DONWLOAD, 50):
		...
		t = time.time()
		for row in table.find_all("div",attrs={'class':'lister-item-image'})[1:]:
			#Gets the actor's image.
			imageLink = row.find('img')
